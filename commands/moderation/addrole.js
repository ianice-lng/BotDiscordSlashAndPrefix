...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (targetHighestRole >= memberHighestRole) {
      return interaction.reply("Tu ne peux pas ajouter ce role");
    }
...
async run(client, message, args){
  ...
  try{
    ...
    if (targetHighestRole >= memberHighestRole) {
      return message.reply("Tu ne peux pas ajouter ce role!");
    }
    if(user.id){
...
async function commandBan(message, member, author, role) {
    member.roles.add(role).then(() => {
        message.reply(`Le role ${role.name} a bien été ajouté à ${member.user.tag}`)
    }).catch(err => {
